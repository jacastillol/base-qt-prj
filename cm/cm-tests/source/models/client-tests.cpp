...
void ClientTests::verifyBillingAddress(const QJsonObject& jsonObject)
{
    ...
    QCOMPARE(billingAddress.value("building").toString(),
             QString("Billing Building"));
    QVERIFY(billingAddress.contains("street"));
    QCOMPARE(billingAddress.value("street").toString(),
             QString("Billing Street"));
    QVERIFY(billingAddress.contains("city"));
    QCOMPARE(billingAddress.value("city").toString(),
             QString("Billing City"));
    QVERIFY(billingAddress.contains("postcode"));
    QCOMPARE(billingAddress.value("postcode").toString(),
             QString("Billing Postcode"));
}
...
void ClientTests::verifySupplyAddress(const QJsonObject& jsonObject)
{
    ...
    QVERIFY(billingAddress.contains("building"));
    QCOMPARE(billingAddress.value("building").toString(),
             QString("Supply Building"));
    QVERIFY(billingAddress.contains("street"));
    QCOMPARE(billingAddress.value("street").toString(),
             QString("Supply Street"));
    QVERIFY(billingAddress.contains("city"));
    QCOMPARE(billingAddress.value("city").toString(),
             QString("Supply City"));
    QVERIFY(billingAddress.contains("postcode"));
    QCOMPARE(billingAddress.value("postcode").toString(),
             QString("Supply Postcode"));
}
...
void ClientTests::verifyAppointments(const QJsonObject& jsonObject)
{
    ...
    QCOMPARE(appointment1.value("startAt").toString(),
             QString("2017-08-20T12:45:00"));
    QVERIFY(appointment1.contains("endAt"));
    QCOMPARE(appointment1.value("endAt").toString(),
             QString("2017-08-20T13:00:00"));
    QVERIFY(appointment1.contains("notes"));
    QCOMPARE(appointment1.value("notes").toString(),
             QString("Test appointment 1"));

    QJsonObject appointment2 = (appointments.at(1).toObject());
    QVERIFY(appointment2.contains("startAt"));
    QCOMPARE(appointment2.value("startAt").toString(),
             QString("2017-08-21T10:30:00"));
    QVERIFY(appointment2.contains("endAt"));
    QCOMPARE(appointment2.value("endAt").toString(),
             QString("2017-08-21T11:30:00"));
    QVERIFY(appointment2.contains("notes"));
    QCOMPARE(appointment2.value("notes").toString(),
             QString("Test appointment 2"));
}
...
void ClientTests::verifyAppointments(const QList<Appointment*>& appointments)
{
    ...
    QCOMPARE(appointments.size(), 2);
    QCOMPARE(appointments.at(0)->startAt->value(),
             QDateTime(QDate(2017, 8, 20), QTime(12, 45)));
    QCOMPARE(appointments.at(0)->endAt->value(),
             QDateTime(QDate(2017, 8, 20), QTime(13, 0)));
    QCOMPARE(appointments.at(0)->notes->value(), QString("Test appointment 1"));
    QCOMPARE(appointments.at(1)->startAt->value(),
             QDateTime(QDate(2017, 8, 21), QTime(10, 30)));
    QCOMPARE(appointments.at(1)->endAt->value(),
             QDateTime(QDate(2017, 8, 21), QTime(11, 30)));
    QCOMPARE(appointments.at(1)->notes->value(), QString("Test appointment 2"));
}

void
ClientTests::verifyDefaultAppointments(const QList<Appointment*>& appointments)
{
    QCOMPARE(appointments.size(), 0);
}
...
void ClientTests::verifyContacts(const QJsonObject& jsonObject)
{
    ...
    QCOMPARE(contact1.value("contactType").toInt(),
             static_cast<int>(Contact::eContactType::Email));
    ...
    QCOMPARE(contact2.value("contactType").toInt(),
             static_cast<int>(Contact::eContactType::Telephone));
}
...
void ClientTests::verifyContacts(const QList<Contact*>& contacts)
{
    ...
    QCOMPARE(contacts.at(0)->contactType->value(),
             static_cast<int>(Contact::eContactType::Email));
    QCOMPARE(contacts.at(1)->address->value(), QString("012345678"));
    QCOMPARE(contacts.at(1)->contactType->value(),
             static_cast<int>(Contact::eContactType::Telephone));
}
